...
export async function getVideoInfo(videoId) {
  ...
  const thumb = Array.isArray(basic.thumbnail) ? basic.thumbnail[0] : null;
  const isVerticalThumb = Boolean(thumb && thumb.height && thumb.width && thumb.height > thumb.width);
  const isShortDuration = basic.duration > 0 && basic.duration < 60;
  const isShort = Boolean(basic.is_short) || isShortDuration || isVerticalThumb;
  ...
  return {
    videoId,
    title: basic.title || primaryTitle || '',
    description: basic.short_description || secondaryDescription || '',
    channel: basic.channel?.name ?? '',
    duration: basic.duration ?? 0, // seconds
