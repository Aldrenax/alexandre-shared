...
export async function getVideoInfo(videoId) {
  ...
  let fallback = {};
  if (!basic.duration || !Array.isArray(basic.thumbnail) || basic.thumbnail.length === 0) {
    try {
  ...
  const thumbnails = Array.isArray(basic.thumbnail) && basic.thumbnail.length
    ? basic.thumbnail
    : Array.isArray(fallback.thumbnails)
      ...
      : fallback.thumbnail
        ...
        : [];
  const duration = Number(basic.duration || fallback.duration || 0);
  ...
  const isShort = Boolean(basic.is_short) || isShortDuration || isVerticalThumb || isShortUrl;
