...
export function extractPublishedAt(info, basic) {
  const plausible = (value) => {
    ...
    const earliest = Date.parse('2005-02-14T00:00:00.000Z');
    const latest = Date.now() + 24 * 3_600_000;
    return Number.isFinite(date.getTime()) && date.getTime() >= earliest && date.getTime() <= latest ? date : null;
  };
  const direct = plausible(basic?.publish_date);
  if (direct) return direct;
  const publishedText = info?.primary_info?.published?.text;
  if (publishedText) {
    const relative = plausible(parseRelativeDate(publishedText));
    if (relative) return relative;
  }
  const start = plausible(basic?.start_timestamp);
