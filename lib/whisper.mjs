...
import { existsSync, mkdirSync, unlinkSync, readdirSync, statSync, rmdirSync } from 'node:fs';
...
export function ytDlpNetworkEnv(env = process.env) {
...
export async function ytDlpVideoMetadata(videoId) {
  ...
  ], { timeout: 120_000, captureStdout: true, env: ytDlpNetworkEnv() });
...
export async function whisperTranscribe(videoId, { language = 'fr' } = {}) {
  ...
  try {
    ...
    await run('yt-dlp', [
      ...
      '-o', audioPath,
      `https://www.youtube.com/watch?v=${videoId}`,
    ], { timeout: 480_000, env: ytDlpNetworkEnv() });
    ...
    const { stdout } = await run('python3', [TRANSCRIBE_SCRIPT, audioPath, language], { timeout: 1_800_000 });
    return stdout.trim();
  } finally {
    try {
