...
export function transcriptCachePath(videoId, env = process.env) {
  // Le cache est aussi la frontière entre le Studio OAuth et le Media Engine :
  // Studio y dépose des sous-titres officiels, Hermes ne voit jamais le refresh
  // token. Le repli Whisper conserve le même format texte et le même cache.
  const root = String(
    env.MEDIA_ENGINE_CAPTION_CACHE_DIR ||
    env.WHISPER_TRANSCRIPT_CACHE_DIR ||
    (env.MEDIA_ENGINE_RUNTIME_DIR ? join(env.MEDIA_ENGINE_RUNTIME_DIR, 'cache', 'transcripts') : ''),
  ).trim();
  if (!root) return null;
...
export async function runYtDlpWithRetries(args, {
  ...
  // Un proxy résidentiel est utile contre les protections YouTube, mais il ne
  // doit jamais immobiliser le cycle : après ses tentatives, on essaie le
  // réseau direct sauf désactivation explicite par l'opérateur.
  allowDirectFallback = !/^(?:0|false|no)$/i.test(String(env.YTDLP_ALLOW_DIRECT_FALLBACK ?? 'true')),
  runImpl = run,
  waitImpl = wait,
} = {}) {
  const errors = [];
  const plans = Array.from({ length: attempts }, () => ({ direct: false }));
  // Essayer le direct juste après le premier proxy : attendre toutes les
  // tentatives proxy peut immobiliser un cycle vidéo pendant plusieurs minutes.
  if (allowDirectFallback && env.HTTP_PROXY_URL) plans.splice(1, 0, { direct: true });
...
export async function whisperTranscribe(videoId, { language = 'fr' } = {}) {
  ...
  const audioDownloadTimeout = Math.max(30_000, Number(process.env.YTDLP_AUDIO_TIMEOUT_MS || 90_000));
